# -*- coding: utf-8 -*-
"""
Motore v4: come engine3.py, ma con la CORREZIONE 5 (fascia quality-based).

Problema diagnosticato in v3: la fascia (F1..F4) che una squadra sta
"riempendo" e' decisa SOLO dal conteggio di quanti giocatori ha gia'
comprato in quel ruolo, mai da quanto e' forte il giocatore chiamato in
quel momento. Con 4 fasce invece di 2, questo crea 3 punti di transizione
in cui un giocatore forte (di rank F1, es. titolare) arrivato tardi viene
valutato con il budget-per-slot della fascia bassa che la squadra sta
riempiendo (es. F2/F3), che e' molto piu' piccolo.

Fix: se il giocatore chiamato appartiene (per rank di mercato, cioe' per
FVM rispetto agli altri del suo ruolo) a una fascia MIGLIORE di quella che
la squadra sta riempiendo per conteggio, e il suo FVM e' nettamente sopra
la media della fascia-target (soglia configurabile), il peso con cui si
calcola il bid ceiling riflette anche il suo vero valore di mercato
(media della sua fascia reale), non solo la media della fascia bassa in
cui il conteggio lo starebbe infilando. Il "pot" di budget (target/spent)
resta comunque quello della fascia-target per conteggio: non si sposta
budget da una fascia all'altra, si paga di piu' per uno slot che lo
merita, nei limiti dello spendibile residuo (gia' vincolato altrove).
"""
import random, math
import openpyxl
...
# CORREZIONE 5: soglia oltre la quale un giocatore e' considerato
# "nettamente piu' forte" della fascia-target che la squadra sta riempiendo
# per conteggio, e quindi merita una valutazione allineata alla sua vera
# fascia di mercato invece che a quella bassa del conteggio.
QUALITY_UPGRADE_SOGLIA = 1.3
QUALITY_UPGRADE_WEIGHT_CAP = 6.0
...
def run_auction(players, tiers, seed, my_flags, opp_flags, noise_sigma=0.15, noise_clip=(0.7, 1.4)):
    ...
    def bid_ceiling(t, role, p, market_fascia, flags, pace_league):
        ...
        # CORREZIONE 5 (quality-based fascia): se il giocatore chiamato
        # appartiene, per rank di mercato, a una fascia MIGLIORE di quella
        # che la squadra sta riempiendo per conteggio (use_fascia), e il suo
        # FVM e' nettamente sopra la media della fascia-target, si usa un
        # peso allineato alla sua vera fascia di mercato invece che alla
        # media della fascia bassa. Il pot di budget resta quello della
        # fascia-target (nessun trasferimento di budget tra fasce): si paga
        # semplicemente di piu', nei limiti dello spendibile.
        weight_cap = 4.0
