# -*- coding: utf-8 -*-
"""
Motore v2: implementa le 3 correzioni concordate con l'utente sopra engine.py.

3) Scarsita' (mercato) e Quality Scarcity ora premiano SOLO quando i giocatori
   scarseggiano (fino a +30% / +50%). Quando sono abbondanti il prezzo resta
   al valore base FVM (niente piu' sconto artificiale sotto il punto 1).

4) Titolari/Riserve non e' piu' uno switch binario. Ogni ruolo ha 4 fasce a
   qualita' decrescente (fascia1 = titolari, fascia2/3/4 = riserve di
   qualita' via via minore), ciascuna con un proprio target di budget, cosi'
   da avere sempre "una o due riserve buone" e non solo scarti puri. Esempio
   difensori (8 slot, 3 titolari): 3+2+2+1.

5) La Competition Pressure sulle fasce di riserva (2/3/4) si applica SOLO se
   il giocatore chiamato ha un FVM nettamente piu' alto (soglia configurabile,
   default +30%) della media delle riserve rimaste nella STESSA fascia -
   niente piu' aste inutili su riempitivi intercambiabili. Sui titolari
   (fascia1) la pressione competitiva resta sempre attiva come prima.
"""
import random, math
import openpyxl
...
RISERVE_TIER_SPLIT = [0.5, 0.3, 0.2]  # come si divide il budget "riserve" tra fascia2/3/4
COMP_PRESSURE_RISERVA_SOGLIA = 1.3     # una riserva "vale" la pressione competitiva solo se FVM >= 1.3x la media delle riserve rimaste nella sua fascia
...
def build_fasce(role):
    """Ritorna la lista [(nome, count_per_squadra, share_del_budget_ruolo), ...] per un ruolo."""
    tit_n = TITOLARI[role]
    riserve_n = ROSTER[role] - tit_n
    # divide le riserve in fino a 3 sotto-fasce, il piu' possibile equilibrate e
    # "front-loaded" (piu' peso alle fasce migliori) - stessa formula usata
    # nell'esempio dei difensori (8, tit=3) -> 2,2,1
    ...
    # se una fascia ha 0 slot, la sua quota di budget va redistribuita (evita di
    # "perdere" budget quando riserve_n e' piccolo, es. portieri)
    for i in range(1, 4):
...
def build_tiers(players):
    ...
    for role in ROSTER:
        ...
        pool_n = cum  # totale giocatori "nel pool" di ruolo (tutte le fasce)
...
def run_auction(players, tiers, seed, my_flags, opp_flags, noise_sigma=0.15, noise_clip=(0.7, 1.4)):
    ...
    def bid_ceiling(t, role, p, market_fascia, flags, pace_league):
        ...
        if flags["auction_state"]:
            # trova la prima fascia non ancora completata dalla squadra
            use_fascia = None
            ...
            use_fascia = fasce[0][0] if market_fascia == "floor" else market_fascia  # bookkeeping soltanto
            target = role_budget
        ...
        if flags["market_scarcity"]:
            ...
            if avail_ratio < 1:  # CORREZIONE 3: solo premio se scarso, mai sconto se abbondante
        ...
        if flags["quality_tiers"] and p["id"] in tiers[role]["top_buono_ids"]:
            ...
            if q_ratio < 1:  # CORREZIONE 3: idem, solo premio se scarso
        ...
        if flags["competition_pressure"]:
            ...
            apply_pressure = True
            if not is_titolari:
                # CORREZIONE 5: sulle riserve, solo se questo giocatore e'
                # nettamente piu' forte della media delle riserve rimaste
                # nella sua stessa fascia
