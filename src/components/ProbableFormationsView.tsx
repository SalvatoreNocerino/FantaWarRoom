...
import { PageHeader, Card, SectionHeader, Badge } from './ui';
      ...
      {team && (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {team.titolari.map((p, i) => (
                <div key={i} className="flex items-center gap-2 bg-surface-2 rounded-lg px-3 py-2">
                  <span className="w-11 shrink-0 font-mono text-[10px] font-bold text-accent">{p.ruolo}</span>
          ...
          <Card className="p-6 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {team.panchina.map((name, i) => (
                <span key={i} className="px-2.5 py-1 rounded-lg bg-surface-2 text-xs text-ink-soft">
                  {name}
                </span>
              ))}
            </div>
